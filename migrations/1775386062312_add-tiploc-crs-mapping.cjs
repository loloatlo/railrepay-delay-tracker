...
exports.up = async (pgm) => {
  ...
  pgm.createTable(
    { schema: 'delay_tracker', name: 'tiploc_crs_mapping' },
    {
      tiploc_code: {
        type: 'varchar(7)',
        primaryKey: true,
        // TIPLOC codes are at most 7 characters (NaPTAN/CORPUS standard).
        // NOT NULL implied by PRIMARY KEY.
      },
      crs_code: {
        type: 'char(3)',
        notNull: true,
        // CRS (Computer Reservation System) code, always 3 uppercase characters.
        // Used by National Rail customer-facing systems and journey.confirmed payloads.
      },
      station_name: {
        type: 'varchar(255)',
        // Human-readable station name. Nullable — informational only, not used
        // by the Sequential Leg Walk algorithm. Avoids NOT NULL constraint
        // to allow partial loads and future NaPTAN sync without breaking existing rows.
      },
      created_at: {
        type: 'timestamptz',
        notNull: true,
  ...
  // with multiple TIPLOC entries for different platforms/areas). This index
  // supports efficient lookups in both directions.
  pgm.createIndex(
    { schema: 'delay_tracker', name: 'tiploc_crs_mapping' },
    ['crs_code'],
    {
      name: 'idx_tiploc_crs_mapping_crs_code',
      // Not UNIQUE — one CRS code can have multiple TIPLOC entries.
    }
  );
...
exports.down = async (pgm) => {
  // Drop the index first (cascade would handle this, but explicit is clearer).
  pgm.dropIndex(
    { schema: 'delay_tracker', name: 'tiploc_crs_mapping' },
    ['crs_code'],
    { name: 'idx_tiploc_crs_mapping_crs_code', ifExists: true }
  );

  // Drop the table — CASCADE handles any views or dependent objects.
  pgm.dropTable(
    { schema: 'delay_tracker', name: 'tiploc_crs_mapping' },
